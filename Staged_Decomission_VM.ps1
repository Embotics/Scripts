<#
Usage, Migrate a Decomissioning VM, takes the VM out of the portal usrs view and relocates it to a decomission location in vmware. 
Requires Powershell V4, Embotics vCommander Rest API V2.8 and PowerCLi 6.5 or higher
...
##############################################################################################################
    ...
    $CredFile = 'c:\scripts\vCommandercred.xml'  #  Credential file to access your vCommander
    $VIServer = 'yourvCenterServer'              #  vCenter Server name must be reachable (DNS)
    $vCredFile = 'c:\scripts\vcentercred.xml'    #  vCenter Cred File
    ...
    $DecomRpool = "Decomission"    #Resource pool to move vm to
...
#Move VM Based on above logic
    ...
    $performFoldermove = VMware.VimAutomation.Core\Move-VM -VM $vm.Name -Destination $Folder
    $Clustername = ($VM | Select Name, @{N="Cluster";E={VMware.VimAutomation.Core\Get-Cluster -VM $_}}).cluster.name
    $performRPmove = VMware.VimAutomation.Core\Move-VM -VM $vm.Name -Destination ((VMware.VimAutomation.Core\Get-ResourcePool -Name $DecomRpool -Location $Clustername)[0])
