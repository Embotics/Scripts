<#
.SYNOPSIS
Script to enable Termination protection on an AWS EC2 instance
...
.DESCRIPTION
 ...
 powershell.exe C:\Scripts\enable_termiationprotection.ps1 '#{target.remoteId}' '#{target.id}' '#{target.region.name}'
...
#vCommander
    ...
    $Attrib = "Enable Termination Protection"               #
#AWS
    $AWSAccesskey = '000000000000'
    $AwsSecretkey = '00000000000000000000000000000'
...
#Set Elastic IP
    Try{$InstanceTermination = (Get-EC2InstanceAttribute -InstanceId $Instance -Region $Region -Attribute DisableApiTermination).DisableApiTermination
		if ($InstanceTermination -eq $False) {
            ...
            Start-sleep 2
            $TerminationValue = (Get-EC2InstanceAttribute -InstanceId $Instance -Region $Region -Attribute DisableApiTermination).DisableApiTermination
			Write-Host "$Instance : Termination flag: $TerminationValue" -ForegroundColor Green
		} 
        }
        Catch{
